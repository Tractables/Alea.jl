...
# The fundamental datatype in Alea is `Dist{Bool}`, which represents a
...
@assert pr(x) ≈ Dict(false => 0.3, true => 0.7)
@assert pr(x | true) ≈ Dict(true => 1.0)

# We can use &, |, ^, ! to perform computation on DistBools.
@assert pr(flip(0.5) & flip(0.1))[true] ≈ 0.05

# We can find conditional probabilities using the `evidence` keyword arg
x = flip(0.5)
y = flip(0.1)
# Pr[X & Y given X]
@assert pr(x & y, evidence=x)[true] ≈ 0.1
...
@assert pr(ite_example)[true] ≈ 0.95
...
net = network()
@assert isapprox(pr(net)[true], 0.99975, rtol=0.001)
